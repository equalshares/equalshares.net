...
Jeśli co najmniej 5% wyborców zagłosuje na ten projekt _i na żadne inne projekty_, wówczas projekt ten na pewno zostanie wybrany przez metodę równych udziałów.
...
Jeśli co najmniej $n \cdot \text{koszt}(P)/B$ wyborców zagłosowało na $P$ i jeżeli żaden z tych wyborców nie zagłosował na żaden inny projekt, to $P$ zostanie wybrany przez metodę równych.

<details className="proof">
<summary>Dowód</summary>

Każdy z wyborców, który zagłosował jedynie na $P$, dostaje na początku kwotę równą co najmniej $B/n$. (Może to być więcej niż $B/n$, ponieważ [uzupełniamy](../realizacja/completion) wynik jeśli nie wykorzystuje on w pełni dostępnego budżetu).
...
Metoda równych udziałów kończy działanie dopiero wtedy, gdy zwolennicy żadnego niewybranego jeszcze projektu nie mają już wystarczających środków, aby pokryć koszt takiego projektu. To oznacza, że $P$ musi zostać wybrany, ponieważ wyborcy, 
...
Przy zastosowaniu standardowej metody głosowania w budżecie obywatelskim (która po prostu wybiera najpopularniejsze projekty do wyczerpania budżetu), nie mamy takich gwarancji.
...
że każdy wyborca głosuje na wszystkich członków tej partii, którą popiera. Okzauje się, że jeśli zastosujemy metodę równych udziałów do tak skonstruowanej instancji budżetu obywatelskiego, to wynik jej działania
