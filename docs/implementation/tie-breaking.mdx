...
These simulations suggest that it is exceptionally infrequent for a tie to occur that needs to be broken using randomness.
...
Most of the ties found in our simulation occur in election in the city of Wrocław, which involve a fairly small number of voters.
...
The simulation was performed in January 2023 using election data form Pabulib based on PB election that used approval voting, which concerns 480 datasets from the Polish cities of Gdansk, Warsaw, Wrocław, and Zabrze. When excluding the Wrocław elections, the simulation involves 383 datasets. 
