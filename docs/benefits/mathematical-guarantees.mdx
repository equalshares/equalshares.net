...
In this case, one can prove that the Method of Equal Shares will select all four projects.
...
In the mathematical guarantees described above, we assumed that the group of voters voted for exactly the same set of projects. In practice, however, even groups of voters with strong agreement will not always vote for exactly the same set of projects. Therefore, it is interesting to specify to what extent the Method of Equal Shares will select projects that are supported by a group of voters with high but not perfect agreement.

Let us consider a PB election based on approval voting. 
Suppose that a group of $t$ voters all approve the projects in the set $T = \{P_1, P_2, \ldots, P_k\}$, but some of the voters also approve some other projects not in $T$.
Write $\text{cost}(T) = \text{cost}(P_1) + \text{cost}(P_2) + \ldots + \text{cost}(P_k)$ for the total cost of the projects in $T$.
Assume that $\text{cost}(T)\leqslant t/n \cdot B$.
Then, the Method of Equal Shares guarantees that at least one of the $t$ voters will be sufficiently represented in the outcome.
