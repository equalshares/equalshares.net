...
To be concrete, let's say that the overall budget is <CurrencySymbol/>100, and there are 10 voters.
Thus, in Step 1, each voter is allocated a budget share of <CurrencySymbol/>10.
We can represent this in the following picture, where each voter is represented by a bar of height <CurrencySymbol/>10.

<BudgetBars budgets={[10,10,10,10,10,10,10,10,10,10]} showPayments={false} />

Suppose there is a project that costs <CurrencySymbol/>25, and it received 5 votes from the 5 voters on the left.
Then we can spread the cost equally, with each supporter paying <CurrencySymbol/>5.
We can show this in the picture as follows:

<BudgetBars budgets={[10,10,10,10,10,10,10,10,10,10]} payments={[5,5,5,5,5,0,0,0,0,0]} />

After some time, the budget shares of some voters may be diminished, in which case a perfectly equal split of the cost is not possible anymore.
In this case, we do it as indicated in the following picture. You can change the cost of the project using the slider.
