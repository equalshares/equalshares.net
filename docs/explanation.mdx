...
To be concrete, let's say that the overall budget is <CurrencySymbol/>300, and there are 10 voters.
...
Or for another example, suppose that the two voters on the left have only <CurrencySymbol/>5 left to spend (because they each spent a total of <CurrencySymbol/>25 in prior rounds already).
